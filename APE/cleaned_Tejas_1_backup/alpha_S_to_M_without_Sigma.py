...
def main_func(data, \
	...
	R_start, T_start = icp_test(S[:,0:num_sampled_sens_points_ICP].T, M_sampled.T, tree_M_sampled)
...
##############################################################################
	...
	def mycallback(m, where):
		...
		if 	SolRepeatCounter < 100:
			...
			if where == GRB.Callback.MIPNODE:
				...
				R_after_ICP,T_after_ICP = icp_test(transformed_for_ICP.T, M_sampled.T, tree_M_sampled)
				elapsed = time.time() - t
	...
	for iii in range(number_of_solutions):
		...
		if iii < m.SolCount:
			...
			T1_input_invalid = R_T_to_4X4 ((R_bucket_input,T_bucket_input))
			error_mat = T1_input_invalid.dot(np.linalg.inv(gt))
			...
			print np.linalg.norm(angle_error),"angle_error_before_bucket"
			bucket_list_ang_err.append(np.linalg.norm(angle_error))
